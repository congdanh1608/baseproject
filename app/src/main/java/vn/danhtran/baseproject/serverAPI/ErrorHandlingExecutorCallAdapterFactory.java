...
/**
 * Created by danhtran on 07/11/2016.
 */
        ...
        public void onResponse(final Call<T> poCall, final Response<T> poResponse) {
            if (poResponse.isSuccessful()) {
                mCallbackExecutor.execute(new Runnable() {
                    @Override
                    public void run() {
                        mDelegate.onResponse(poCall, poResponse);
                    }
                });
            } else {
...
//                }
                ...
                mCallbackExecutor.execute(new Runnable() {
                    @Override
                    public void run() {
                        mDelegate.onFailure(poCall, RetrofitException.httpError(poResponse.raw().request().url().toString(), poResponse, mRetrofit));
                    }
                });
            }
        }
        ...
        public void onFailure(final Call<T> poCall, final Throwable poThrowable) {
            ...
            mCallbackExecutor.execute(new Runnable() {
                @Override
                public void run() {
                    mDelegate.onFailure(poCall, loFinalException);
                }
            });
        }
    }
